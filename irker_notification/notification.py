...
class IrkerNotifcationPlugin(Component):
    ...
    def pre_process_request(self, req, handler):
        """Handles requests containing subscription related actions like subscribe and unsubscribe."""
        if not req.session.authenticated:
            return handler
        if req.method == 'GET' and 'subscribe' in req.args:
            if req.args.get('subscribe') == 'Subscribe':
                if not SubscriptionHandler.is_session_subscribed_for_ticket_changes(self.env, req.session.sid):
                        SubscriptionHandler.add_subscription(self.env, self.log, req.session.sid, 'ResourceChangeIrcSubscriber')
                prev_subs = req.session.get('subscriptions','')
                if len(prev_subs) == 0:
                    req.session['subscriptions'] = req.path_info
                else:
                    req.session['subscriptions'] = req.session['subscriptions']  + ', %s' % req.path_info
                req.session.save()
                add_notice(req, _('You have subscribed successfully!'))
            else:
                updated_subscriptions = [x.strip() for x in req.session['subscriptions'].split(',') if not x.strip() == req.path_info]
    ...
    def filter_stream(self, req, method, filename, stream, data):
        ...
        if filename == 'ticket.html':
            stream = stream | Transformer('body//div[@class="trac-content "]').prepend(HTML(self._get_button_html(req)))
            self.log.debug('#IrkerNotifcationPlugin filter_stream')

        if filename == 'wiki_view.html':
            stream = stream | Transformer('body//div[@id="wikipage"]').prepend(HTML(self._get_button_html(req)))
            self.log.debug('#IrkerNotifcationPlugin filter_stream')
        return stream
    ...
    def _get_button_html(self, req):
        ...
        if SubscriptionHandler.is_session_subscribed_to(self.env, req.session.sid, req.path_info):
            button = button % (_('Unsubscribe'), _('Unsubscribe from IRC notifications'))
        else:
            button = button % (_('Subscribe'), _('Subscribe to IRC notifications'))
